package com.demo.test;

import java.util.Scanner;

import com.demo.beans.Product;
import com.demo.services.ProductService;
import com.demo.services.ProductServiceImpl;

public class TestProductServices {

	public static void main(String[] args) {
		Scanner sc = new Scanner(System.in);
		ProductService pservice = new ProductServiceImpl();
		int ch=0;
		pservice.startConnection();
		do {
			System.out.println("------------------Product Menu--------------------");
			System.out.println("1. Add new Product\n" + "2. Modify Product\n" + "3. Delete Product\n"
					+ "4. Display All Product\n" + "5. Find by ID\n" + "6. Display in sorted Order\n" + "7. Exit\n"
					+ "-------------------------------------------------\n" + "Enter your Choice:");
			ch = sc.nextInt();
			switch (ch) {
			case 1 -> {
				boolean status = pservice.addNewProduct();
				if (status) {
					System.out.println("Product Added Succefully...!");
				} else {
					System.out.println("Something Went Wrong...!");
				}
			}
			case 2 -> {
				System.out.println("Enter Product Id:");
				int id = sc.nextInt();
				System.out.println("Enter New Price:");
				double price = sc.nextDouble();
				boolean status  = pservice.modifysalary(id,price);
				if(status) {
					System.out.println(id+" -Product price modified successfully");
				}else {
					System.out.println("Not updated...");
				}
				}
			case 3->{
				System.out.println("Enter Product ID that you want to delete:");
				int id = sc.nextInt();
				boolean status = pservice.deleteById(id);
				if(status) {
					System.out.println("Deleted Successfully...");
				}else {
					System.out.println("");
				}
			}
			case 4->{
				pservice.display();
			}
			case 5->{
				System.out.println("Enter Product ID:");
				int id = sc.nextInt();
				Product p = pservice.findById(id);
				if(p!=null)
				System.out.println(p);
				else
					System.out.println("Didn't find the Product...");
			}
			case 7 -> {
				System.out.println("Thank You for Visiting...!");
				sc.close();
			}
			}
		} while (ch != 7);

	}

}
