...
public class AccountService {   //Parent
	...
	public static double mini;

	public AccountService() {
		super();
	}

	public AccountService(String accType, String fName, String lName, long mob, String email, double bal) {
		super();
		FName = fName;
		LName = lName;
		this.mob = mob;
	...
	public String toString() {
		return "AccountService [FName=" + FName + ", LName=" + LName + ", mob=" + mob + ", email=" + email + ", id=" + id
				+ "]";
	}

	public static AccountService[] addCustomer() {

		AccountService[] custArr = new AccountService[2];
...
//			sc.nextLine();
			...
			if (flag) {
				AccountService a =new AccountService(accType, fname, lname, mob, email, bal);
				custArr[i] = a;
			} else {
				break;
			}
...
//		} 
	...
	public static void displayCusts(AccountService[] a) {

		for (AccountService i : a) {
			if (i != null) {
	...
	public static AccountService[] searchCustomer(AccountService[] a, String fname) {
		AccountService[] custDetail = new AccountService[a.length];
		int count = 0;
		for (AccountService ob : a) {
	...
	static AccountService a = new AccountService();
	
	static Saving sObj = new Saving(); 

	public static AccountService getAccountById(String accId, AccountService[] custArr) {

		for (AccountService a : custArr) {
		...
		do {
			...
			case 2:
				...
				if (check == 1) {
					...
					System.out.println("You can only withdraw " + ((account.getBal()) - (AccountService.mini)) + " money.");
				}else {
					break;
				}
				break;
